  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        let statsData = null;
        try {
          const statsRes = await fetch(`${API_BASE_URL}/api/tickets/stats`, { headers });
        ...
        let ticketsData = [];
        try {
          const ticketsRes = await fetch(`${API_BASE_URL}/api/tickets`, { headers });
  ...
  return (
    <div className="space-y-10 animate-fade-in">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        {/* Removed Green Access Badge as requested */}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        {/* SYSTEM HEALTH REPLACEMENT: TICKET CATEGORIES */}
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-100 flex flex-col min-h-[300px]">
          <h3 className="text-lg font-bold mb-6 text-slate-800 flex items-center gap-2">
            <FileText className="text-[#064e3b]" size={20} /> Top Categories
          </h3>
          ...
          <div className="space-y-6">
            {categories.length > 0 ? (
              categories.map((cat, index) => (
                <div key={index}>
                  <div className="flex justify-between text-xs font-black text-slate-500 mb-2 uppercase tracking-wide">
                    <span>{cat.name}</span>
                    <span>{cat.count}</span>
                  </div>
                  <div className="w-full bg-slate-100 h-2.5 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{ width: `${totalTickets > 0 ? (cat.count / totalTickets) * 100 : 0}%` }}
                    ></div>
                  </div>
                </div>
              ))
